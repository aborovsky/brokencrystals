...
export class AppController {
  ...
  getSecrets(): Record<string, string> {
    ...
    // Instead, they should be fetched from a secure storage or environment variables.
    const secrets = {
      codeclimate: process.env.CODECLIMATE_REPO_TOKEN ? 'Available' : 'Unavailable',
      facebook: process.env.FACEBOOK_TOKEN ? 'Available' : 'Unavailable',
      google_b64: process.env.GOOGLE_B64 ? 'Available' : 'Unavailable',
      google_oauth: process.env.GOOGLE_OAUTH ? 'Available' : 'Unavailable',
      google_oauth_token: process.env.GOOGLE_OAUTH_TOKEN ? 'Available' : 'Unavailable',
      heroku: process.env.HEROKU_TOKEN ? 'Available' : 'Unavailable',
      hockey_app: process.env.HOCKEY_APP_TOKEN ? 'Available' : 'Unavailable',
      outlook: process.env.OUTLOOK_WEBHOOK ? 'Available' : 'Unavailable',
      paypal: process.env.PAYPAL_ACCESS_TOKEN ? 'Available' : 'Unavailable',
      slack: process.env.SLACK_TOKEN ? 'Available' : 'Unavailable'
    };
    return secrets;
  }
