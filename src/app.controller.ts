...
export class AppController {
  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      const allowedVariables = { /* define allowed variables here */ };
      const compiledTemplate = dotT.template(text, { varname: Object.keys(allowedVariables) });
  ...
  getSecrets(): Record<string, string> {
    // Secrets should be retrieved from a secure source, such as environment variables
    const secrets = {
